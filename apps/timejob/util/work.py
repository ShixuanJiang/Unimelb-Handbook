        ...
        if not is_start:
            ...
            if not is_exists:
                if work_name == "test_work":
                    ...
                    JobController.objects.filter(work_name=work_name).update(job_id=job_id)  # 更新Job_id
                elif work_name == "test_job":
                    scheduler_tool = SchedulerManager()
                    job_id = scheduler_tool.add_job_by_interval(test_job_func, ['ok find'], 5)
                    scheduler_tool.start()
                    JobController.objects.filter(work_name=work_name).update(job_id=job_id)  # 更新Job
            else:
